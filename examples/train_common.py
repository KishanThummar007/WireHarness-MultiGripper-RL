...
# ---------------- settings ----------------
TAG              = "harness_sac"
TOTAL_EPISODES   = 40      # ADDITIONAL episodes to run this session
DETAIL_EVERY     = 1       # print per-STEP detail every N episodes (0 = never)
CHECKPOINT_EVERY = 1       # save policy + buffer + state every N episodes
MAX_EP_STEPS     = 150     # episode length cap. A trained policy reaches the connector
                           #   in ~120 env steps; anything beyond that is wasted compute.
RESUME           = True    # continue from a previous run if a checkpoint exists
LOG_FILE         = "training.log"
PLOT_FILE        = "learning_curve.png"
PLOT_EVERY       = 1       # redraw the learning curve every N episodes (0 = only at the end).
                           #   1 = live view: open learning_curve.png and refresh while training.
...
# =============================================================================
#  PRETTY CONSOLE  —  colours, episode banner, live progress bar + ETA
# =============================================================================
# ANSI colours work in Windows Terminal / VS Code / PowerShell 10+.
# os.system("") switches on VT100 processing on Windows; harmless elsewhere.
if os.name == "nt":
    os.system("")            # enable ANSI colour on Windows
 
# Windows consoles default to cp1252, which has no block characters, so the
# progress bar would raise UnicodeEncodeError. Try UTF-8, else fall back to ASCII.
try:
...
def _printable(ch):
    try:
        ch.encode(getattr(sys.stdout, "encoding", None) or "ascii")
        return True
    except Exception:
        return False
 
 
BAR_FULL, BAR_EMPTY = ("\u2588", "\u2591") if _printable("\u2588") else ("#", "-")
...
class PlainFormatter(logging.Formatter):
    """Strips colour codes so the LOG FILE stays readable."""
...
def progress_bar(done, total, elapsed, width=30):
    """Coloured bar + percentage + elapsed + ETA for the WHOLE training run."""
    frac = 0.0 if total <= 0 else min(1.0, done / total)
    fill = int(width * frac)
    bar = f"{C.GRN}{BAR_FULL * fill}{C.GREY}{BAR_EMPTY * (width - fill)}{C.RESET}"
    eta = (elapsed / done) * (total - done) if done else 0.0
    return (f"  [{bar}] {C.BOLD}{done}/{total}{C.RESET} episodes "
            f"({frac*100:3.0f}%)   elapsed {C.CYN}{fmt_hms(elapsed)}{C.RESET}"
            f"   ETA {C.YEL}{fmt_hms(eta)}{C.RESET}")
...
_fh = logging.FileHandler(LOG_FILE, mode="a" if _RESUMING else "w", encoding="utf-8")
_fh.setFormatter(PlainFormatter("%(asctime)s  %(message)s", "%H:%M:%S"))
...
class EpisodeLogger(BaseCallback):
    ...
    def __init__(self, total_episodes, detail_every, checkpoint_every, resume_state=None):
        ...
        self.t_session = time.time()            # for the progress bar / ETA
        self.t_ep = time.time()
        self.ep_start_pos = None
        self.ep_start_dist = None
    ...
    def _on_step(self) -> bool:
        ...
        g, t, dist = info["grip"], info["target"], info["distance"]
        if self.ep_start_pos is None:                 # first step of the episode
            self.ep_start_pos = np.array(g, float)
            self.ep_start_dist = dist
            self._last_dist = dist
 
        show = self.detail_every and (self.ep % self.detail_every == 0)
        if show and not self._banner_shown:
            self._banner_shown = True
            logger.info("")
            logger.info(f"{C.CYN}{'=' * 96}{C.RESET}")
            logger.info(f"{C.BOLD}{C.CYN} EPISODE {self.ep} / "
                        f"{self.start_ep + self.total_episodes}{C.RESET}"
                        f"    target {C.MAG}[{t[0]:.2f} {t[1]:.2f} {t[2]:.2f}]{C.RESET}"
                        f"    start distance {C.WHT}{dist:.3f} m{C.RESET}"
                        f"    max {MAX_EP_STEPS} steps")
            logger.info(f"{C.CYN}{'=' * 96}{C.RESET}")
 
        if show:
            moved = float(np.linalg.norm(np.array(g, float) - self.ep_start_pos))
            closer = dist < self._last_dist
            dcol = C.GRN if closer else C.RED
            arrow = "v" if closer else "^"
            rcol = C.GRN if reward > 0 else (C.YEL if reward > -1.5 else C.RED)
            logger.info(
                f" {C.GREY}step{C.RESET} {self.ep_steps:4d}  "
                f"{C.GREY}action{C.RESET}[{action[0]:+.2f} {action[1]:+.2f} {action[2]:+.2f}]  "
                f"{C.GREY}position{C.RESET}[{g[0]:6.2f} {g[1]:6.2f} {g[2]:6.2f}]  "
                f"{C.GREY}to-target{C.RESET} {dcol}{dist:6.3f} m {arrow}{C.RESET}  "
                f"{C.GREY}moved{C.RESET} {moved:5.3f} m  "
                f"{C.GREY}reward{C.RESET} {rcol}{reward:+7.3f}{C.RESET}")
        self._last_dist = dist
        ...
        if done:
            ...
            ep_time = time.time() - self.t_ep
            tag = f"{C.GRN}{C.BOLD} PASS {C.RESET}" if success else f"{C.RED}{C.BOLD} FAIL {C.RESET}"
            rcol = C.GRN if self.ep_reward > 0 else C.RED
            logger.info(
                f"{tag} {C.BOLD}episode {self.ep}{C.RESET}"
                f"  |  {self.ep_steps:4d} steps"
                f"  |  reward {rcol}{self.ep_reward:+9.2f}{C.RESET}"
                f"  |  start {self.ep_start_dist:.3f} -> final {C.WHT}{info['distance']:.3f} m{C.RESET}"
                f"  |  radius {radius:.2f}"
                f"  |  took {C.CYN}{fmt_hms(ep_time)}{C.RESET}")
            logger.info(progress_bar(self.ep - self.start_ep + 1,
                                     self.total_episodes,
                                     time.time() - self.t_session))
            ...
            # live learning curve: redraw so you can watch progress DURING training
            if PLOT_EVERY and (self.ep - self.start_ep) % PLOT_EVERY == 0:
                plot_curve(self.rewards_hist, self.success_hist,
                           self.dist_hist, self.radius_hist, PLOT_FILE, quiet=True)
...
def plot_curve(rewards, success, dists, radii, path, quiet=False):
    import matplotlib
    ...
    plt.close(fig)          # ESSENTIAL when plotting every episode: otherwise figures
                            # accumulate and matplotlib leaks memory / warns after ~20
    if not quiet:
