...
# --- motion tuning ------------------------------------------------------------
MAX_STEP_M = 0.12          # commanded gripper displacement per env step
MAX_LEAD   = 1.0 * MAX_STEP_M   # how far the persistent setpoint may run AHEAD of the gripper.
                                # Measured on this env (substeps=20):
                                #   lead = 3.0x -> 47% speed ripple, action alignment +0.10
                                #   lead = 1.0x ->  4% speed ripple, action alignment +0.15
                                # i.e. 1.0x is BOTH smoother and more responsive. A large lead
                                # pins the setpoint against the clamp, so the gripper just
                                # chases it at constant speed and single actions stop mattering.
...
def make_env(render_mode=None):
    ...
    return HarnessPickEnv(
        ...
        # SPEED: with smooth_setpoint the PID no longer has to "arrive" each env step, so
        # substeps=20 tracks BETTER (76%) than the old substeps=50 without it (68%) at 2.5x
        # less physics cost. See MAX_STEP_M / MAX_LEAD above for the motion tuning.
        substeps=20, max_step_m=MAX_STEP_M,
